...
def page_upload():
    ...
    if st.session_state.get('_loaded_df') is not None:
        _finalize(st.session_state['_loaded_df'],
...
def _finalize(df, fixed_cols, remaps, source):
    ...
    for w in validate_dataframe(df):
        st.warning(f"⚠️ {w}")
    if remaps:
        remap_html = "".join(f"<div class='alert-box alert-blue'>{r}</div>" for r in remaps)
...
def build_popup(row):
    ...
    c = status_color(s); e = status_emoji(s)
    d = row.get('dist_km', 0)
    ...
    if map_type == 'heatmap':
        ...
        for _,row in df.iterrows():
            ...
            folium.CircleMarker([row['lat'],row['long']],
                ...
                color=fc if j=='M' else 'white',
                weight=1.5 if j=='M' else 3,
                fill=True, fill_color=fc, fill_opacity=0.85,
...
def _vis_approver(df):
    ...
    n_terima  = int(df['is_terima'].sum())
    n_tolak   = int(df['is_tolak'].sum())
    ...
    if not agg.empty:
        ...
        with cl:
            ...
            fig.update_layout(height=380, showlegend=False,
                              xaxis_title='', yaxis_title='Jumlah Absensi')
            st.plotly_chart(fig, use_container_width=True)
        with cr:
            fig = px.pie(
...
def _run_preprocessing(df_raw: pd.DataFrame, config: dict):
    ...
    mask_m = df['jenis']=='M'; mask_p = df['jenis']=='P'
    df['menit_telat'] = np.nan; df['menit_cepat'] = np.nan
    ...
    df['jam']=df['tanggal_kirim'].dt.hour; df['menit']=df['tanggal_kirim'].dt.minute
    df['jam_desimal']=df['jam']+df['menit']/60.0; df['weekday']=df['tanggal_kirim'].dt.weekday
    df['tanggal']=df['tanggal_kirim'].dt.date; df['timestamp_num']=df['tanggal_kirim'].astype(np.int64)//10**9
    log("✅ STEP 2: Time features")
    df=df.dropna(subset=['lat','long'])
    df['lat_rad']=np.radians(df['lat']); df['long_rad']=np.radians(df['long'])
    log("✅ STEP 3: Koordinat ke radian")
    if not config.get('run_dbscan',True) or not SKLEARN_OK:
        for c in ['cluster_masuk','cluster_pulang','is_noise_masuk','is_noise_pulang',
                  'cluster_size_masuk','cluster_size_pulang']: df[c]=0
        log("⏭️ STEP 4-5: DBSCAN dilewati")
    else:
        eps_rad=config.get('eps_km',0.1)/6371.0; min_smp=config.get('min_samples',3)
        ...
        log(f"✅ STEP 4: DBSCAN — noise masuk:{df['is_noise_masuk'].sum():,}")
        office_locs={}
        for skpd,grp in df[df['jenis']=='M'].groupby('id_skpd'):
            nn=grp[grp['cluster_masuk']!=-1]; src=nn if len(nn)>0 else grp
            if len(nn)>0:
                best=nn['cluster_masuk'].value_counts().idxmax(); src=nn[nn['cluster_masuk']==best]
        ...
        log(f"✅ STEP 5: Centroid {len(office_locs)} SKPD")
    if 'office_lat' not in df.columns:
        oc_s=df[df['jenis']=='M'].groupby('id_skpd')[['lat','long']].median().reset_index()
        oc_s.columns=['id_skpd','office_lat','office_long']
        df=df.merge(oc_s,on='id_skpd',how='left')
    ...
    if not config.get('run_stdbscan',True) or not SKLEARN_OK:
        ...
        st_eps_km=config.get('st_eps_km',0.1); st_eps_hr=config.get('st_eps_hours',1.0)
        st_min_smp=config.get('st_min_samples',3); eps_deg=st_eps_km/111.0; time_scale=eps_deg/st_eps_hr
        def stdbscan(subset):
            result=pd.Series(-1,index=df.index,dtype=int)
            for skpd,grp in subset.groupby('id_skpd'):
                if len(grp)<st_min_smp: continue
                X=grp[['lat','long']].copy(); X['jam_sc']=grp['jam_desimal']*time_scale
    ...
    log(f"✅ STEP 9: Anomaly — HIGH:{rl.get('HIGH',0):,} MED:{rl.get('MED',0):,}")
    keep_cols=['karyawan_id','id_skpd','jenis','tanggal_kirim','tanggal',
...
def page_preprocessing():
    ...
    if run_btn:
        ...
        try:
            progress.progress(10,"Menghitung...")
    ...
    if logs:
        st.markdown("### 📋 Log"); [st.markdown(f"- {lg}") for lg in logs]
    c1,c2,c3,c4=st.columns(4)
